...
@Component({
  selector: 'app-month-output',
  templateUrl: './month-output.component.html',
  styleUrls: ['./month-output.component.scss']
})
export class MonthOutputComponent {
  timeInput: Time[] = [];
  timeList: string[] = [];
  today: string;
  ...
  constructor(private timeService: TimeService) {
    this.timeList = this.getLastMonth();
  ...
  async refresh(){
    ...
    for(var i=0; i<this.timeInput.length; i++){
      if(this.timeList.includes(this.timeInput[i].date)){
